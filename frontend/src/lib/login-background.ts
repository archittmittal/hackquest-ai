...
export const createLoginBackgroundScene = (container: HTMLDivElement) => {
    try {
        ...
        // Animation loop
        let time = 0;
        const posArray = particlesGeometry.getAttribute('position').array as Float32Array;

        const animate = () => {
            requestAnimationFrame(animate);
            time += 0.016;

            (particlesMaterial.uniforms.time as any).value = time;
